<aside id="sidebar"
    ...
    <div class="p-6 flex items-center gap-3">
        <div
            ...
            <img src="{{ asset('storage/' . $webConfig->logo) }}" class="w-full h-full object-contain}" alt="">
        </div>
        <div>
            <h1 class="text-xl font-bold text-white tracking-tight">{{$webConfig->name}}</h1>
            <p class="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">{{$webConfig->judul}}</p>
        </div>
    </div>
    ...
    <nav class="flex-1 px-4 space-y-1 overflow-y-auto mt-4">
        ...
        @if(auth()->user()->isAdmin())
            ...
            <p class="text-[10px] font-bold text-slate-500 px-4 py-2 uppercase tracking-widest">Master data</p>
            <x-navigation href="kategori" icon="list">
                Kategori
            </x-navigation>
            <x-navigation href="admin/produk" icon="package-search">
                Produk
            </x-navigation>
            <p class="text-[10px] font-bold text-slate-500 px-4 py-2 uppercase tracking-widest">Setting</p>
        ...
        @elseif(auth()->user()->isCustomer())
            <p class="text-[10px] font-bold text-slate-500 px-4 py-2 uppercase tracking-widest">Produk</p>
            <x-navigation href="produk" icon="package-search">
                Produk
            </x-navigation>
            <x-navigation href="cart" icon="shopping-cart">
                Keranjang
            </x-navigation>
            <x-navigation href="riwayat" icon="history">
                Transaksi
            </x-navigation>
